...
/**
 * Builds the Word document structure for an Executive Ministry Briefing.
 */
export async function exportMinistryBriefingDocx(
  projects: MPLADProject[],
  meta: {
    summaryCr: string;
    exposureCr: string;
    ...
    sections: [
      {
        ...
        children: [
          new Paragraph({
            children: [
              new TextRun({
                text: 'GOVERNMENT OF INDIA • MINISTRY OF STATISTICS AND PROGRAMME IMPLEMENTATION',
                bold: true,
                size: 20,
                color: '475569',
                font: 'Calibri',
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: 'EXECUTIVE MPLADS VIGILANCE BRIEFING',
                bold: true,
                size: 34,
                color: '0F172A',
                font: 'Calibri',
            ...
            children: [
              new TextRun({
                text: `Generated on: ${new Date().toLocaleDateString('en-IN', {
                  ...
                  month: 'long',
                  year: 'numeric',
                })} | National Health Score: ${meta.nationalScore}/100`,
                size: 20,
                color: '64748B',
                font: 'Calibri',
              }),
          ...
          new Paragraph({
            text: '1. Executive Portfolio Highlights',
            heading: HeadingLevel.HEADING_1,
          ...
          new Table({
            width: { size: 100, type: WidthType.PERCENTAGE },
            rows: [
              createKeyValueRow('Monitored Portfolio Capital', `₹${meta.summaryCr} Crores`, false),
              createKeyValueRow('National Risk Index', `${meta.nationalScore} / 100 (Safe Bounds)`, true),
              createKeyValueRow('P0 Urgent Action Interventions', `${meta.p0Count} Works`, false),
              createKeyValueRow('Total Financial Exposure Identified', `₹${meta.exposureCr} Crores`, true),
              createKeyValueRow('Total Projects Analyzed', `${projects.length} Works`, false),
            ],
          }),
          new Paragraph({
            text: '2. Targeted Administrative Directives',
            heading: HeadingLevel.HEADING_1,
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: '• Directive A: Mandatory geo-spatial reconciliation for all works where financial expenditure exceeds physical milestones by over 25%.',
                size: 20,
                font: 'Calibri',
              }),
            ],
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: '• Directive B: Implement automated Bill of Quantities (BoQ) benchmarking against state Schedule of Rates before technical sanction approvals.',
                size: 20,
                font: 'Calibri',
              }),
            ],
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: '• Directive C: Cross-verify duplicate GPS coordinates with PMGSY and AMRUT scheme registries to prevent double billing.',
                size: 20,
                font: 'Calibri',
              }),
            ],
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: 'This briefing is generated by the MPLADS VigilAI Decision Intelligence Platform for administrative oversight and audit prioritization. In accordance with statutory guidelines, risk findings do not constitute final determinations of misconduct without physical field inquiry.',
                italics: true,
                size: 18,
                color: '64748B',
                font: 'Calibri',
  ...
  const blob = await Packer.toBlob(doc);
  const dateStr = new Date().toISOString().slice(0, 10);
  triggerDownload(blob, `MPLADS_Ministry_Executive_Briefing_${dateStr}.docx`);
}
