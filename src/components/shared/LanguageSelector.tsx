  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <button
        ...
        className="px-2.5 sm:px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 border border-slate-200/90 text-slate-800 text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer shadow-2xs focus:ring-2 focus:ring-indigo-500 focus:outline-hidden"
        title="Switch Language / भाषा बदलें / ભાષા બદલો"
      >
        <Globe className="w-3.5 h-3.5 text-indigo-600 shrink-0" />
        <span className="hidden lg:inline text-slate-600 font-medium">Language:</span>
        <span className="text-sm leading-none">{currentLanguageInfo.flag}</span>
        <span className="font-bold hidden sm:inline">{currentLanguageInfo.nativeName}</span>
        <span className="font-bold sm:hidden">{currentLanguageInfo.code.toUpperCase()}</span>
        <ChevronDown
      ...
      {isOpen && (
        <div
          ...
          className="absolute top-full right-0 mt-2 w-52 bg-white rounded-2xl border border-slate-200 shadow-2xl py-1.5 z-50 animate-in fade-in zoom-in-95"
        >
          <div className="px-3.5 py-1.5 text-[10px] uppercase font-bold tracking-wider text-slate-400 border-b border-slate-100 flex items-center justify-between">
            ...
            <span className="font-normal text-[9px] text-slate-400">भाषा / ભાષા</span>
          </div>
            ...
            {languages.map((lang) => {
              ...
              return (
                <button
                  ...
                  className={`w-full px-3 py-2 rounded-xl text-left text-xs flex items-center justify-between transition-colors cursor-pointer focus:outline-hidden focus:ring-1 focus:ring-indigo-500 ${
                    isSelected
