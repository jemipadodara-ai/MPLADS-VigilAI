        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-0.5 rounded-md bg-indigo-50 border border-indigo-200 text-indigo-700 text-[11px] font-bold uppercase tracking-wider">
                {t('phrases.Works Registry', 'Works Registry')}
              </span>
              <span className="text-slate-300">•</span>
              <span className="text-slate-500 text-xs font-medium">
                {projects.length} {t('phrases.Total Monitored Works', 'Total Monitored Works')}
              </span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">
              {t('projects.registryTitle', 'Projects Master Registry')}
            </h1>
            <p className="text-xs sm:text-sm text-slate-500">
              {t('phrases.Filterable registry of sanctioned infrastructure works with risk tiers, expenditure, and detailed risk reasons.', 'Filterable registry of sanctioned infrastructure works with risk tiers, expenditure, and detailed risk reasons.')}
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5 self-start md:self-auto">
            ...
            <span className="text-xs font-semibold text-slate-500 mr-1">
              {t('phrases.Showing', 'Showing')} <strong className="text-slate-900 font-mono">{filteredProjects.length}</strong> {t('phrases.of', 'of')} {projects.length} {t('phrases.works', 'works')}
            </span>
            ...
            >
              ...
              <span>{isExportingWord ? t('projectDetail.generating', 'Exporting...') : t('projectDetail.exportWord', 'Export Word Report')}</span>
            </button>
            ...
            >
              ...
              <span>{t('phrases.Export CSV', 'Export CSV')}</span>
            </button>
          </div>
        </div>
        ...
        <div className="pt-4 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 items-center">
          ...
          <div className="lg:col-span-4 relative">
            ...
            <input
              ...
              type="text"
              placeholder={t('projects.searchPlaceholder', 'Search by code, title, ward, contractor...')}
              value={searchTerm}
          ...
          <div className="lg:col-span-3">
            ...
            >
              <option value="All">{t('phrases.All Districts / Constituencies', 'All Districts / Constituencies')}</option>
              {uniqueWards.map((w) => (
          ...
          <div className="lg:col-span-2">
            ...
            >
              <option value="All">{t('phrases.All Risk Levels', 'All Risk Levels')}</option>
              <option value="High">{t('common.high', 'High')} (70-100)</option>
              <option value="Medium">{t('common.medium', 'Medium')} (40-69)</option>
              <option value="Low">{t('common.low', 'Low')} (0-39)</option>
            </select>
          </div>
          ...
          <div className="lg:col-span-2">
            ...
            >
              ...
              <option value="In Progress">{t('In Progress', 'In Progress')}</option>
              <option value="Delayed">{t('Delayed', 'Delayed')}</option>
              <option value="Completed">{t('Completed', 'Completed')}</option>
              <option value="Under Investigation">{t('Under Investigation', 'Under Investigation')}</option>
              <option value="Sanctioned">{t('Sanctioned', 'Sanctioned')}</option>
            </select>
          </div>
          ...
          <div className="lg:col-span-1 flex justify-end">
            <button
              ...
              title={t('common.reset', 'Reset all filters')}
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-xs overflow-hidden">
        {filteredProjects.length === 0 ? (
          <div className="py-16 text-center space-y-3">
            ...
            <h3 className="text-base font-bold text-slate-900">{t('phrases.No matching projects found', 'No matching projects found')}</h3>
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              {t('phrases.Try adjusting your search query, location selection, or risk filter.', 'Try adjusting your search query, location selection, or risk filter.')}
            </p>
            <button
              onClick={handleResetFilters}
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/80 text-[11px] uppercase tracking-wider text-slate-500 font-bold">
                  <th className="py-3.5 px-4 sm:px-6">{t('phrases.Project Reference & Title', 'Project Reference & Title')}</th>
                  <th className="py-3.5 px-4">{t('phrases.Sanctioned Budget', 'Sanctioned Budget')}</th>
                  <th className="py-3.5 px-4">{t('phrases.Expenditure', 'Expenditure')}</th>
                  <th className="py-3.5 px-4 min-w-[130px]">{t('phrases.Progress', 'Progress')}</th>
                  <th className="py-3.5 px-4">{t('common.riskScore', 'Risk Level')}</th>
                  <th className="py-3.5 px-4 min-w-[260px]">{t('phrases.Main Risk Reason', 'Main Risk Reason')}</th>
                  <th className="py-3.5 px-4 text-right">{t('common.action', 'Action')}</th>
                </tr>
              </thead>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 font-bold text-slate-900 whitespace-nowrap font-mono">
                        ₹{(project.sanctionedAmountLakhs || 0).toFixed(2)} {t('common.lakhs', 'Lakh')}
                      </td>

                      {/* Expenditure */}
                      <td className="py-3.5 px-4 whitespace-nowrap font-mono">
                        <span className="font-semibold text-slate-800">
                          ₹{(project.expenditureAmountLakhs || 0).toFixed(2)} {t('common.lakhs', 'Lakh')}
                        </span>
                      ...
                      <td className="py-3.5 px-4 text-right whitespace-nowrap">
                        <button
                          ...
                          title={t('common.viewDetails', 'Open project details')}
                        >
